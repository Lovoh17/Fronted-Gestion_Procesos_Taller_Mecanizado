...
export default {
  ...
  setup() {
    ...
    const toggleSidebar = () => {
      ...
      localStorage.setItem('coordinatorSidebarCollapsed', isCollapsed.value)
    }
    ...
    const handleLogout = async () => {
      try {
        console.log('🚪 [CoordinatorSidebar] Iniciando proceso de logout...')

        // Usar el método logout del store que ya implementaste
        authStore.logout()

        console.log('✅ [CoordinatorSidebar] Logout completado, redirigiendo...')

        // Redirigir al login o página principal
        await router.push('/')

      } catch (error) {
        console.error('❌ [CoordinatorSidebar] Error durante el logout:', error)
    ...
    const loadSidebarState = () => {
      const savedState = localStorage.getItem('coordinatorSidebarCollapsed')
      if (savedState !== null) {
